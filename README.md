...
| `site.host.builder` | **Optional**, `static` hosts only. `quartz-book`: the platform builder builds and deploys this book (BOOK-ONE-TO-QUARTZ §0). Absent means the builder leaves the book alone. A builder book's branches must be readable with no credentials, and `github-facts` checks that. |
