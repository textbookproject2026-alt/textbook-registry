...
[`docs/`](docs) is the **platform owner's** operating documentation: the
infrastructure inventory, the scheduled-jobs health check, the runbook for adding
and retiring a book, the CMS relay and the author's app. Start at
[`docs/README.md`](docs/README.md). A book maintainer's guides live in their own
book's `docs/`, not here.

**Status: step 5.** Five things read the registry now:
...
redeployed from a specific commit of this repo. Delivery as it stands (DESIGN §2b planned the rest):
...
| CMS auth relay (Cloudflare Worker) | **not delivered.** `ALLOWED_DOMAINS` is edited by hand; generating it from `cms.host` (DESIGN step 5a) is not built | when someone edits the Worker variable |
...
**Portal.** `.github/workflows/portal.yml` is the same mechanism for the portal: after
`validate` passes on `main` it fires a Cloudflare Pages deploy hook on the
`textbook-portal` project, then polls `https://<platform.portal.domain>/version.txt`
until it shows the merged commit, failing after ten minutes. It also runs every six
hours, at a different minute from `deploy.yml`. It needs the repository secret
**`PORTAL_DEPLOY_HOOK`** (the Pages deploy hook URL, a credential like the one above).
The repository variable `PORTAL_VERSION_URL` redirects the poll during setup only; it
is unset, and should stay unset, because polling the apex is what proves readers see
the change.

What a red run of either workflow means, and what to do: `docs/SCHEDULED-JOBS.md`.
...
| `site.host` (`obsidian-publish`) | `site_id` (the Publish site's 32-hex id, which every page embeds publicly) and `publish_host`. Optionally `builder` and `project`, together (below). |
...
| `site.host.builder` | **Optional**, book hosts only, never the portal. `quartz-book`: the platform builder builds and deploys this book (BOOK-ONE-TO-QUARTZ §0). On a `static` host, that is the site readers see. On an `obsidian-publish` host, before the cutover, it is **a preview only**: the builder deploys to the Cloudflare Pages project named by `project` (`<project>.pages.dev`), and readers are still served by Publish at `site.domain`, which stays the book's only address. `builder` and `project` go together on a Publish host, and the cutover switches the host kind and keeps both. Absent means the builder leaves the book alone. A builder book's branches must be readable with no credentials, and `github-facts` checks that. |
...
|---|---|---|
| `validate` → `registry` | every push and PR | JSON is malformed or has duplicate keys; schema violation (missing field, unknown key, bad slug format, malformed URL or host, unknown status, host kind, provider, builder, payer or dark reason); duplicate slug, domain, content repo, CMS host, legacy origin or provider project (a Publish book's preview project counts as a Cloudflare Pages one); a Publish host with `builder` but no `project`, or the reverse; a domain that is also a legacy origin; an alias that is any book's domain, alias or legacy origin; the portal domain used by a book; `drafts_branch` equals `live_branch`; null domain on a non-preview book; a domain, CMS host or portal CMS host that is a bare platform suffix (`pages.dev`…); a domain on a shared suffix unless the book is `static` and `preview`; a hostname more than one label under `platform.portal.book_parent`; a portal `host.project` that is also a book's, on the same provider; `site.dark` on a book that isn't `live`; `paid_by: maintainer` with no `maintainer.github`; a slug removed or renamed |
| `validate` → `github-facts` | every push and PR | a content repo that is missing, private, or moved; a live or drafts branch that doesn't exist; a template or extras repo that is missing (a book with `editions: null` has no template to check); a builder book (`site.host.builder`) whose live or drafts branch can't be read anonymously. It only **warns** if a domain, CMS host or preview URL doesn't currently answer |
...
- docs, `textbook` `01b83a4`: `docs.cms-allowed-domains`, `docs.cms-homepage`,
  `docs.infrastructure.cms-relay`, `docs.infrastructure.template-preview`. These read
  `OAUTH-SETUP.md` and `docs/INFRASTRUCTURE.md` in the vault, which moved to this
  repo's `docs/` (split and rewritten) on 22 Sep 2026. The retirement is verified by the
  vault's `docs/README.md` pointing at `textbook-registry/docs/INFRASTRUCTURE.md`. The
  relay allowlist those docs stood in for is still not checkable by anything.
